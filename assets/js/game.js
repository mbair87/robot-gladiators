// Game States
// "WIN" - Player robot has deeated all enemy-robots
...
var fight = function (enemy) {
  ...
  while (playerInfo.health > 0 && enemy.health > 0) {
    ...
    if (isPlayerTurn) {
    if (fightOrSkip()) {
      //if true, leavve fight by breaking loop
      break;
    }
...
var startGame = function () {
 ...
 playerInfo.reset();
  ...
  for (var i = 0; i < enemyInfo.length; i++) {
    if (playerInfo.health > 0) {
      //let player know what round they are in, remember that arrays start at 0 so needs 1 added
      window.alert("Welcome to Robot Gladiators! Round " + (i + 1));
 
      //pick new enemy to fight based on the index of
